                        ...
                        <div class="table w-full">
                            <div class="table-row-group">
                                <div v-for="contract in contracts" :key="contract.id" class="table-row">
                                    <div class="table-cell px-4 py-2 " title="№">{{ contract.id }}</div>
                                    <div class="table-cell px-4 py-2 " title="Регистрационный номер автомобиля"><inertia-link :href="route('cars.edit',contract.car)">{{ contract.car.registration_number }}</inertia-link></div>
                                    <div class="table-cell px-4 py-2 " title="Дата контракта">{{ contract.date }}</div>
                                    <div class="table-cell px-4 py-2 " title="Цена контракта">{{ contract.car.price }}</div>
                                    <div class="table-cell px-4 py-2 " title="ИНН покупателя"><inertia-link :href="route('customers.edit',contract.customer)">{{ contract.customer.inn }}</inertia-link></div>
                                    <div class="table-cell px-4 py-2 " title="ИНН продавца"><inertia-link :href="route('users.edit',contract.user)">{{ contract.user.inn }}</inertia-link></div>
                                    <div class="table-cell px-4 py-2 text-right">
                                        <inertia-link :href="route('contracts.edit', contract)" title="Редактировать"
                                        ...
                                        <jet-secondary-button @click.native="confirmContractDeletion(contract)"
                                        ...
                                        <jet-dialog-modal :show="confirmingContractDeletion"
                                            ...
                                            <template #content>
                                                ...
                                                <div class="mt-4">
                                                    <jet-input type="password" class="mt-1 block w-3/4"
                                                               ...
                                                               @keyup.enter.native="deleteContract"/>
                                            ...
                                            <template #footer>
                                                <jet-secondary-button
                                                    @click.native="confirmingContractDeletion = false">
                                                    Отмена
                                                </jet-secondary-button>

                                                <jet-danger-button class="ml-2" @click.native="deleteContract"
                                ...
                                <!--                                <jet-pagination :links="contracts.links"></jet-pagination>-->
                            </div>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    data() {
        return {
            confirmingContractDeletion: false,
            deleting: false,
    ...
    methods: {
        confirmContractDeletion(contract) {
            this.deleteForm.password = '';
            this.contract = contract;

            this.confirmingContractDeletion = true;
        ...
        deleteContract() {
            this.deleteForm.post(route('contracts.destroy', this.contract), {
                preserveScroll: true
            }).then(response => {
                if (!this.deleteForm.hasErrors()) {
                    this.confirmingContractDeletion = false;
                }
            })
        },
    },
